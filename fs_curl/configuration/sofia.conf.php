...
/*
    private function write_gateways($profile_id) {
        //$query = "SELECT * FROM sofia_gateways WHERE sofia_id=$profile_id ORDER BY gateway_name, gateway_param";
        $query = "SELECT * FROM sofia_gateways WHERE sofia_id=$profile_id ORDER BY gateway_name";
        $gateway_array = $this -> db -> queryAll($query);
        $gateway_count = count($gateway_array);
        //$this -> comment_array($gateway_array);
        if (FS_PDO::isError($gateway_array)) {
		...
		for ($i=0; $i<$gateway_count; $i++) {
			$var_count=0;
			$this -> xmlw -> startElement('gateway');
			$this -> xmlw -> writeAttribute('name', $gateway_array[$i]['gateway_name']);
