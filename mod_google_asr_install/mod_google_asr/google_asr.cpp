...
extern "C" {
     switch_status_t google_speech_load(char *google_application_crdential,int log_level_parameter,char * uuid_Pass)
    {
      autoload.google_application_crdentials = google_application_crdential;
      autoload.log_level = log_level_parameter;
      autoload.uuid = uuid_Pass;
     // switch_channel_set_variable(channel, "transcription_data", json);
      switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE,"crdentials_loading from configuration file: %s\n", autoload.google_application_crdentials);
      return SWITCH_STATUS_SUCCESS;
    }
    switch_status_t google_speech_init(char *google_application_crdential,int log_level_parameter,char * uuid_Pass) 
    {
       static char * gcsServiceKeyFile = google_application_crdential;

       switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE,"google_speech_init 1: %s\n", google_application_crdential);
       switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE,"google_speech_init 2 %s\n", autoload.google_application_crdentials);

       switch_channel_t *user_channel;
        switch_core_session_t *lsession = NULL;
      lsession = switch_core_session_locate(autoload.uuid);
      if(lsession)
      {
       user_channel = switch_core_session_get_channel(lsession);
       if(user_channel != NULL)
       switch_channel_set_variable(user_channel, "GOOGLE_APPLICATION_CREDENTIALS", gcsServiceKeyFile);
      }
      //switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE,"crdentials_loading: %s\n", gcsServiceKeyFile);
      if (gcsServiceKeyFile) {
        try {
          auto creds = grpc::GoogleDefaultCredentials();
        } catch (const std::exception& e) {
          switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,"Error initializing google api with provided credentials in %s: %s\n", gcsServiceKeyFile, e.what());
    ...
    switch_status_t google_speech_session_init(switch_core_session_t *session, responseHandler_t responseHandler, 
      ...
      struct cap_cb *cb;
      int err;
      switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,"in google_speech_session_init function ........");
      ...
      try {
	    ...
	    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO,"Streamer init done.");
